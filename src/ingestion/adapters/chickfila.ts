...
function itemName($: cheerio.CheerioAPI, firstCell: Cheerio<Element>): string {
  return (
    ...
    firstCell.clone().children().remove().end().text().replace(/\s+/g, " ").trim()
  );
}
...
function nutritionValues($: cheerio.CheerioAPI, cells: Element[]): Array<string> {
  return cells.slice(1).map((cell) => {
    ...
    return boundValue.text().replace(/\s+/g, " ").trim();
  });
}
...
function normalizedItem(args: {
  ...
  return {
    ...
    fatG: numeric(fatG),
    saturatedFatG: numeric(saturatedFatG),
    transFatG: numeric(transFatG),
    fiberG: numeric(fiberG),
    sugarG: numeric(sugarG),
    sodiumMg: numeric(sodiumMg),
    cholesterolMg: numeric(cholesterolMg),
...
export function parseChickfila(document: SourceDocument): NormalizedFood[] {
      ...
      .each((_rowIndex, outerRow) => {
        ...
        itemTable
          ...
          .each((_subIndex, subRow) => {
            ...
            const subName = $(subCells[0]).text().replace(/\s+/g, " ").trim();
  ...
  const uniqueFoods = new Map<string, NormalizedFood>();
  for (const food of foods) {
    const existing = uniqueFoods.get(food.sourceItemId);
    if (!existing) {
      uniqueFoods.set(food.sourceItemId, food);
      continue;
    }
    if (nutritionKeys.some((key) => existing[key] !== food[key])) {
      throw new Error(`Official source contains conflicting nutrition for ${food.name} (${food.servingSize ?? "no size"})`);
    }
    if (!existing.categories.includes(food.category)) existing.categories.push(food.category);
  }
